...
if(!empty($_POST['export_style'])) {
	...
	$export_dir = get_home_path() . 'demo-files/';
	...
	if ( $stylefolder ) {
        ...
        if(!is_dir($export_dir.'stock/')){
            mkdir($export_dir.'stock/', 0777, true);
        }


	$default_content = array();
	$post_types      = array( 'attachment', 'themo_room', 'post', 'page', 'mphb_room_type' );
			...
			foreach ( $meta as $meta_key => $meta_val ) {
				if (
					...
					(
						...
						strpos( $meta_key, 'ehf_' ) === false &&
						strpos( $meta_key, 'th_' ) === false &&
						strpos( $meta_key, '_mphb_' ) === false &&
						strpos( $meta_key, 'mphb_' ) === false &&
